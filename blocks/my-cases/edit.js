...
export default function Edit( props ) {
	 ...
	 * @return {void}
	 */
	const mutAryItem = ( newval, key, aryName ) => {
		const temp = [ { ...attributes[ aryName ][ 0 ] } ];
		temp[ 0 ][ key ] = newval;
		...
		return (
			<Panel>
				<PanelBody title="Text">
					...
					{ display[ 0 ].columns &&
						display[ 0 ].columns.map( ( v ) =>
							renderControlObj(
								columnControls[ v.value ],
								'display'
							)
						) }
					{ errorControls.map( ( v ) =>
						renderControlObj( v, 'text' )
					) }
				</PanelBody>
			</Panel>
		);
